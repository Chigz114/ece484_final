...
from ..datasets.generation import normalize_rgb, pose_to_camera_matrix
from ..estimation.ekf import PoseEKF
from ..perception.npe import atomic_json_save, predict_poses
...
def _rgb_u8(image: Any) -> np.ndarray:
    ...
    if array.ndim != 3 or array.shape[2] != 3:
        raise ValueError(f"renderer returned shape {array.shape}; expected HxWx3 RGB")
...
def _pose_error_metrics(poses: np.ndarray, truth_states: np.ndarray) -> dict[str, Any]:
    if len(poses) == 0:
        return {
            "samples": 0,
...
class VisualLoopResult:
    ...
    def validate_alignment(self) -> None:
        r"""Validate the discrete-time contract of every stored array.
        ...
        if len(self.observations) > len(self.states):
            raise ValueError(
                "an observation cannot be later than the final truth state"
            )
    ...
    def metrics_dict(self) -> dict[str, Any]:
        ...
        truth_jitter = (
            float(
                np.std(
                    np.linalg.norm(np.diff(truth_at_observation[:, :3], axis=0), axis=1)
                )
                * 100.0
            )
        ...
        return {
            ...
            "raw_npe": _pose_error_metrics(self.observations, truth_at_observation),
    ...
    for step_index in range(max_steps):
        ...
        try:
            if estimator == "raw":
                ...
                if ekf_mahalanobis is not None and not np.isfinite(ekf_mahalanobis):
        ...
        try:
            ...
            if control.shape != (4,) or not np.isfinite(control).all():
                raise ValueError(
                    "controller returned a non-finite or malformed control"
                )
            tolerance = 1e-12
            if (
                ...
                raise ValueError(
                    "controller returned a command outside configured limits"
                )
        except Exception as exc:
    ...
    *,
    ...
    with temporary_npz.open("wb") as handle:
        np.savez_compressed(
            ...
            ekf_update_accepted=np.asarray(result.ekf_update_accepted, dtype=np.bool_),
