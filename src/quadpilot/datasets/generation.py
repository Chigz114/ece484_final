...
class ReproDatasetGenerator:
    ...
    def _initialize_output(self, *, target_samples: int, maximum_failures: int) -> None:
    ...
    def _validate_output_topology(self) -> bool:
        if self.output_dir.is_symlink() or not self.output_dir.is_dir():
            raise ValueError(
                f"resume output must be a real directory: {self.output_dir}"
            )
        allowed_root = {
        ...
        if unexpected_root:
            raise ValueError(f"unexpected files in resume output: {unexpected_root}")
        required_root = {
            self.images_dir.name,
            self.records_path.name,
            self.progress_path.name,
        ...
        if self.images_dir.is_symlink() or not self.images_dir.is_dir():
            raise ValueError(
                f"resume images path must be a real directory: {self.images_dir}"
            )
    ...
    def _load_records(self) -> list[dict[str, Any]]:
        ...
        for line_number, line in enumerate(text.splitlines(), start=1):
            ...
            if not (
                ...
                raise ValueError(
                    f"sample {sample_id} pose must contain six finite floats"
                )
            attempt = self._strict_nonnegative_int(record["attempt"], field="attempt")
            if attempt <= previous_attempt:
                raise ValueError(
                    "sample attempts must be positive and strictly increasing"
                )
            previous_attempt = attempt
            if "annotations" in record:
                if (
                    not isinstance(record["annotations"], dict)
                    or not record["annotations"]
                ):
                    raise ValueError(
    ...
    def _validate_progress(
        ...
        *,
        ...
        attempts = self._strict_nonnegative_int(progress["attempts"], field="attempts")
        successes = self._strict_nonnegative_int(
            progress["successes"], field="successes"
        )
        failures = self._strict_nonnegative_int(progress["failures"], field="failures")
    ...
    def generate(
        ...
        *,
        ...
        attempts = 0
        failures = 0
        output_is_empty = not self.output_dir.exists() or (
        ...
        with self.records_path.open("a", encoding="utf-8", newline="\n") as record_file:
            while len(records) < successful_samples:
                ...
                attempts += 1
                try:
                    rendered = self.renderer.render_rgb(pose_to_camera_matrix(pose))
                    rgb = normalize_rgb(
                        rendered,
                ...
                if sample.annotations:
                    record["annotations"] = _json_normalized(dict(sample.annotations))
                record_file.write(
                    json.dumps(
                        record,
                        separators=(",", ":"),
