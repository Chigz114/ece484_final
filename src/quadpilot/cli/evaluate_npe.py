...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--predictions-jsonl", help="Optional per-sample decoded predictions"
    )
    ...
    *,
    ...
    for images, targets, keys in loader:
        ...
        if collect_predictions:
            ...
            position_errors, yaw_errors = pose_error_vectors(
                outputs, targets, normalizer
            )
...
def main(argv: list[str] | None = None) -> int:
    ...
    data_dirs = args.data_dir or [
        source["path"] for source in checkpoint["dataset"]["sources"]
    ]
    ...
    if dataset_matches:
        if args.split_manifest:
            ...
            if sibling_manifest.is_file():
                manifest = create_or_load_split_manifest(
                    sibling_manifest, dataset, create=False
                )
            else:
                manifest = checkpoint["split_manifest"]
                validate_split_manifest(manifest, dataset)
        if manifest.get("manifest_sha256") != checkpoint["split_manifest"].get(
            "manifest_sha256"
        ):
    ...
    evaluated_sources = [
        ...
        if source.get("source_id") in evaluated_source_ids
    ]
    ...
    if existing and not args.overwrite:
        raise FileExistsError(
            f"refusing to overwrite existing evaluation outputs: {existing}"
        )
