...
DEFAULT_MANIFEST = REPOSITORY_ROOT / "configs" / "assets" / "manifest.json"
    ...
    for track, track_info in selected.items():
        run_root = (
            ...
            if args.run_dir is not None
                ...
                track_info.get("runtime", {}).get(
                    "renderer_run_dir",
                    args.asset_root / track / "splatfacto" / track_info["run"],
                )
            ).expanduser()
        )
        track_ok = True
        ...
        for relative, expected in track_info["files"].items():
            ...
            if expected_size is not None and actual_size != expected_size:
                print(f"  BAD_SIZE {relative}: {actual_size} != {expected_size}")
                track_ok = False
                continue
