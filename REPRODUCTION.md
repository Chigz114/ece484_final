...
This document records what is actually reproducible from the checked-in source.
It deliberately separates verified results from claims that still depend on
missing data or model checkpoints.
...
python scripts/reproduce_oracle_visual.py --track all --max-steps 1200
...
`repro_baselines/oracle_strict.json`. Generated trajectories and detailed gate
...
The public repository omits the three rendered GSplat/NeRF scenes, rendered NPE
datasets, trained NPE checkpoints and gate-focused training assets. The next
milestones are therefore:
...
`repro_assets/manifest.json`. Check presence quickly, or verify full hashes, with:

```powershell
python scripts/check_repro_assets.py --track all
python scripts/check_repro_assets.py --track circle --hash
```
...
The public checkpoints are not yet recovered, so this milestone is explicitly a
synthetic interface test rather than a neural-network result. Independent 5 cm
...
```powershell
python scripts/reproduce_synthetic_pose.py --track all --seeds 20
```

The frozen 50-seed aggregate is stored in
`repro_baselines/synthetic_pose_seed0_49.json`. Under this deliberately simple
...
wsl.exe -d Ubuntu-22.04 -- /home/chi/UAV/envs/quadpilot-render/bin/python `
  /mnt/f/UAV/ece484_final/scripts/smoke_render_circle.py `
...
```powershell
wsl.exe -d Ubuntu-22.04 -- /home/chi/UAV/envs/quadpilot-render/bin/python `
  /mnt/f/UAV/ece484_final/scripts/generate_repro_npe_dataset.py `
...
byte counts and recovered hashes are recorded in `repro_assets/manifest.json`.
...
All reproduction changes stay on the local `codex/visual-reproduction` branch.
There is no GitHub push or pull-request step in this workflow.
...
wsl.exe --cd /mnt/f/UAV/ece484_final -d Ubuntu-22.04 -- python3 `
  scripts/check_repro_assets.py --track lemniscate `
...
wsl.exe --cd /mnt/f/UAV/ece484_final -d Ubuntu-22.04 -- python3 `
  scripts/check_repro_assets.py --track uturn `
...
wsl.exe -d Ubuntu-22.04 -- /home/chi/UAV/envs/quadpilot-render/bin/python -u `
  /mnt/f/UAV/ece484_final/scripts/generate_repro_gate_dataset.py `
...
wsl.exe -d Ubuntu-22.04 -- /home/chi/miniconda3/envs/mvot-mmaction/bin/python -u `
  /mnt/f/UAV/ece484_final/scripts/train_repro_npe.py `
  --data-dir /home/chi/UAV/quadpilot-data/npe_datasets/circle/uniform_seed42_10000_v1 `
...
wsl.exe --cd /mnt/f/UAV/ece484_final -d Ubuntu-22.04 -- `
  ...
  scripts/generate_repro_npe_dataset.py `
...
wsl.exe --cd /mnt/f/UAV/ece484_final -d Ubuntu-22.04 -- `
  ...
  scripts/verify_repro_npe_dataset.py `
...
The six source hashes embedded in the completed gate contract are:
...
in `repro_assets/manifest.json`. No TEST result was used to select or tune a
checkpoint.
...
wsl.exe --cd /mnt/f/UAV/ece484_final -d Ubuntu-22.04 -- `
  /home/chi/UAV/envs/quadpilot-render/bin/python -u `
  scripts/reproduce_npe_closed_loop.py `
...
wsl.exe --cd /mnt/f/UAV/ece484_final -d Ubuntu-22.04 -- `
  ...
  scripts/verify_repro_closed_loop.py `
  ...
  --manifest /mnt/f/UAV/ece484_final/repro_assets/manifest.json `
...
wsl.exe --cd /mnt/f/UAV/ece484_final -d Ubuntu-22.04 -- `
  env CUDA_VISIBLE_DEVICES= `
  /home/chi/miniconda3/envs/mvot-mmaction/bin/python `
  scripts/compare_repro_teaser_metrics.py `
  /home/chi/UAV/quadpilot-data/closed_loop/lemniscate_launch_finetune_seed42_both_s1200_l2_v1 `
  --track lemniscate --dyn-seed 42 `
  --output /mnt/f/UAV/ece484_final/repro_baselines/teaser_lemniscate_comparison.json
```
...
`repro_baselines/teaser_lemniscate_comparison.json`, SHA-256
...
wsl.exe --cd /mnt/f/UAV/ece484_final -d Ubuntu-22.04 -- `
  /home/chi/miniconda3/envs/mvot-mmaction/bin/python `
  scripts/calibrate_vicon_nerf.py `
  repro_env/hardware-evidence/vicon_nerf_correspondences.json `
  --output repro_env/hardware-evidence/vicon_from_nerf.json `
  --maximum-rmse-m 0.03
```
...
`repro_env/hardware-preflight.template.json` to a run-specific local config,
...
wsl.exe --cd /mnt/f/UAV/ece484_final -d Ubuntu-22.04 -- `
  env CUDA_VISIBLE_DEVICES= `
  /home/chi/miniconda3/envs/mvot-mmaction/bin/python `
  scripts/check_hardware_readiness.py repro_env/hardware-preflight.json
```
