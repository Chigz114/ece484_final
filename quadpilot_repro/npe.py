...
MODEL_OUTPUT_FORMAT = ("x_normalized", "y_normalized", "z_normalized", "sin_yaw", "cos_yaw")
...
def _validated_pose(value: Any, *, context: str) -> tuple[float, float, float, float, float, float]:
...
def _read_source_samples(root: Path, metadata: Mapping[str, Any]) -> list[tuple[str, tuple[float, ...]]]:
    ...
    if manifest_name:
        ...
        with manifest_path.open("r", encoding="utf-8") as handle:
            for line_number, line in enumerate(handle, start=1):
                ...
                if not isinstance(sample_id, int) or sample_id < 0 or sample_id in seen_ids:
                    raise ValueError(f"invalid or duplicate sample_id at {manifest_path}:{line_number}")
                ...
                pose = _validated_pose(record.get("pose"), context=f"{manifest_path}:{line_number} pose")
        ...
        if poses is not None:
            if len(poses) != len(samples):
                raise ValueError("metadata poses and samples manifest have different lengths")
            for index, (_, pose) in enumerate(samples):
                metadata_pose = _validated_pose(poses[index], context=f"metadata pose {index}")
                if not np.allclose(metadata_pose, pose, rtol=0.0, atol=1e-12):
                    raise ValueError(f"metadata pose {index} disagrees with samples manifest")
    else:
        if not isinstance(poses, list) or not poses:
            raise ValueError(f"legacy dataset has no non-empty poses list: {root / 'metadata.json'}")
        expected = {f"frame_{index:05d}.png" for index in range(len(poses))}
        images_dir = root / "images"
        actual = {path.name for path in images_dir.glob("frame_*.png")} if images_dir.is_dir() else set()
        if actual != expected:
        ...
        samples = [
            (f"images/frame_{index:05d}.png", _validated_pose(pose, context=f"metadata pose {index}"))
            for index, pose in enumerate(poses)
        ]

    declared = metadata.get("n_frames")
    if declared is not None and int(declared) != len(samples):
        raise ValueError(f"metadata n_frames={declared} but discovered {len(samples)} samples")
    if not samples:
    ...
    for source_number, directory in enumerate(data_dirs):
        ...
        for sample_number, (relative_image, pose) in enumerate(samples):
            ...
            if not image_path.is_file():
                raise FileNotFoundError(f"image does not exist for sample {sample_number}: {image_path}")
            try:
...
def _split_counts(n_samples: int, ratios: Sequence[float]) -> list[int]:
    ...
    if values.shape != (3,) or np.any(values < 0) or not np.isclose(values.sum(), 1.0, atol=1e-12):
        raise ValueError("train/val/test ratios must be non-negative and sum to one")
    raw = values * n_samples
    counts = np.floor(raw).astype(int)
    for index in np.argsort(-(raw - counts), kind="stable")[: n_samples - int(counts.sum())]:
        counts[index] += 1
    ...
    if n_samples >= len(positive):
        for empty in [index for index in positive if counts[index] == 0]:
            ...
            if not donors:
                break
            donor = max(donors, key=lambda index: (counts[index], values[index], -index))
            counts[donor] -= 1
            counts[empty] += 1
...
def create_or_load_split_manifest(
    ...
    *,
    ...
    for source_id in source_ids:
        source_records = [record for record in dataset.records if record.source_id == source_id]
        ranked = sorted(
            source_records,
            key=lambda record: hashlib.sha256(f"{int(seed)}:{record.key}".encode("utf-8")).digest(),
        )
        ...
        splits["val"].extend(record.key for record in ranked[train_count : train_count + val_count])
        splits["test"].extend(record.key for record in ranked[train_count + val_count :])
...
def validate_split_manifest(manifest: Mapping[str, Any], dataset: DatasetIndex) -> None:
    ...
    if manifest.get("dataset_fingerprint") != dataset.fingerprint:
        raise ValueError("split manifest dataset fingerprint does not match current dataset")
    if manifest.get("fingerprint_mode") != dataset.fingerprint_mode:
        raise ValueError("split manifest fingerprint mode does not match current dataset")
    ...
    if set(all_keys) != expected:
        ...
        raise ValueError(f"split manifest does not cover dataset exactly; missing={missing}, extra={extra}")
...
class PoseNormalizer:
    ...
    @classmethod
    def fit(cls, records: Sequence[DatasetRecord], epsilon: float = 1e-6) -> "PoseNormalizer":
        if not records:
            raise ValueError("cannot fit a pose normalizer without training records")
        positions = np.asarray([record.pose[:3] for record in records], dtype=np.float64)
    ...
    def encode_pose(self, pose: Sequence[float]) -> torch.Tensor:
        ...
        position = (np.asarray(values[:3]) - np.asarray(self.mean)) / np.asarray(self.std)
        yaw = values[5]
        return torch.tensor(
    ...
    def decode_positions(self, normalized_positions: torch.Tensor) -> torch.Tensor:
        mean = torch.as_tensor(self.mean, dtype=normalized_positions.dtype, device=normalized_positions.device)
        std = torch.as_tensor(self.std, dtype=normalized_positions.dtype, device=normalized_positions.device)
...
def _torchvision_model(backbone: str, weights: str) -> tuple[nn.Module, int]:
    ...
    if backbone not in choices:
        raise ValueError(f"unsupported backbone {backbone!r}; expected {sorted(choices)}")
...
def convert_legacy_state_to_normalized_outputs(
    ...
    weight_candidates = [name for name in converted if name.endswith("regressor.7.weight")]
    ...
    if len(weight_candidates) != 1 or len(bias_candidates) != 1:
        raise ValueError("legacy state does not contain one unambiguous regressor.7 output layer")
...
def convert_state_dict_to_legacy_raw_xyz(
    ...
    bias_candidates = [name for name in converted if name.endswith("regressor.7.bias")]
    if len(weight_candidates) != 1 or len(bias_candidates) != 1:
        raise ValueError("normalized state does not contain one unambiguous regressor.7 output layer")
    weight = converted[weight_candidates[0]]
    bias = converted[bias_candidates[0]]
    if weight.ndim != 2 or weight.shape[0] != 5 or bias.shape != (5,):
...
def pose_error_vectors(
    ...
    position_cm = torch.linalg.vector_norm(predicted_pose[:, :3] - target_pose[:, :3], dim=1) * 100.0
    yaw_deg = torch.abs(wrapped_angle_difference(predicted_pose[:, 3], target_pose[:, 3])) * (180.0 / math.pi)
...
class MetricAccumulator:
    ...
    def __init__(self) -> None:
        self.count = 0
        self.loss_sums = {name: 0.0 for name in ("total", "position", "orientation", "orientation_norm")}
    ...
    def update(
        ...
        components = pose_loss_components(prediction.detach(), target.detach(), loss_config)
        position, yaw = pose_error_vectors(prediction.detach(), target.detach(), normalizer)
        batch_size = int(prediction.shape[0])
        self.count += batch_size
        for name, values in components.items():
            self.loss_sums[name] += float(values.double().sum().cpu())
        self.position_errors_cm.extend(float(value) for value in position.double().cpu())
    ...
    def compute(self) -> dict[str, Any]:
        ...
        return {
            ...
            "loss": {name: value / self.count for name, value in self.loss_sums.items()},
...
def _rng_state_on_cpu(value: Any, *, label: str) -> torch.Tensor:
    ...
    if normalized.dtype != torch.uint8 or normalized.ndim != 1:
        raise ValueError(f"{label} RNG state must be a one-dimensional torch.uint8 tensor")
...
def restore_rng_state(state: Mapping[str, Any]) -> None:
    ...
    if torch.cuda.is_available() and state.get("torch_cuda") is not None:
        ...
        torch.cuda.set_rng_state_all(
            [_rng_state_on_cpu(item, label=f"CUDA[{index}]") for index, item in enumerate(cuda_states)]
        )
...
def validate_repro_checkpoint(checkpoint: Mapping[str, Any]) -> None:
    if checkpoint.get("schema_version") != CHECKPOINT_SCHEMA_VERSION:
        raise ValueError(f"unsupported NPE checkpoint schema: {checkpoint.get('schema_version')}")
    required = {
        "model_config",
        "model_state_dict",
        "normalizer",
    ...
    if missing:
        raise ValueError(f"NPE checkpoint is missing required provenance/state: {missing}")
