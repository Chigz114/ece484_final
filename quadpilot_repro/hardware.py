...
def check_hardware_readiness(config_path: Path) -> dict[str, Any]:
    ...
    record("schema_version", config.get("schema_version") == 1, str(config.get("schema_version")))
    stage = config.get("stage")
    record("stage", stage == "bench_prop_off", f"stage={stage!r}; only bench_prop_off is enabled")
    ...
    if calibration is not None:
        ...
        record("calibration.accepted", acceptance.get("passed") is True, str(acceptance))

    intrinsics = loaded_evidence.get("camera_intrinsics")
    if intrinsics is not None:
        values = [intrinsics.get(key) for key in ("fx", "fy", "cx", "cy", "width", "height")]
        numeric = all(isinstance(value, (int, float)) and np.isfinite(value) for value in values)
    ...
    topic_values = [topics.get(name) for name in required_topics] if isinstance(topics, dict) else []
    topics_ok = (
        len(topic_values) == len(required_topics)
        and all(isinstance(value, str) and value.startswith("/") for value in topic_values)
        and len(set(topic_values)) == len(topic_values)
    )
    record("topics", topics_ok, str(dict(zip(required_topics, topic_values, strict=False))))
    ...
    record("frames", frames == frame_contract, f"required={frame_contract}; actual={frames}")
    ...
    safety_ok = isinstance(safety, dict) and all(safety.get(name) is True for name in required_safety)
    record("manual_safety_checks", safety_ok, str({name: safety.get(name) for name in required_safety}))
