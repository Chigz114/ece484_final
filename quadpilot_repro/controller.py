...
class LegacyVisionControlCore:
    ...
    def _check_gate_crossing(self, position: np.ndarray) -> tuple[str | None, float | None]:
        ...
        radial_error = float(
            np.linalg.norm(relative - distance_to_plane * normal)
        )
        ...
        if current_side != previous_side and previous_side != 0.0:
            ...
            if radial_error <= self.gate_radius * self.pass_radius_multiplier:
                ...
                self.pass_events.append(
                    (self._step_index, gate.name, radial_error)
                )
    ...
    def _trajectory_control(
        ...
        acceleration_body = np.array(
            [
                acceleration_world[0] * cos_yaw
                + acceleration_world[1] * sin_yaw,
                -acceleration_world[0] * sin_yaw
                + acceleration_world[1] * cos_yaw,
    ...
    def step(
        ...
        *,
        ...
        if self.completed:
            ...
            return ControlStep(
                np.zeros(4), True, None, target, self.lap_count
            )

        position = observation[:3]
        yaw = float(observation[3])
        velocity = self._estimate_velocity(
            position, velocity_hint, velocity_estimate
        )
        if self._current_trajectory is None:
