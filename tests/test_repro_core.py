...
from quadpilot_repro.controller import LegacyVisionControlCore
from quadpilot_repro.dynamics import body_acceleration_to_world, step_dynamics
...
from quadpilot_repro.estimation import PoseEKF
from quadpilot_repro.evaluation import evaluate_ordered_gates
from quadpilot_repro.simulation import run_oracle_simulation, run_pose_simulation
from quadpilot_repro.renderer import PoseTransform
from quadpilot_repro.tracks import TRACKS
from quadpilot_repro.trajectory import generate_gate_transition
...
class IncomingGateSideContractTests(unittest.TestCase):
    ...
    def test_schema_rejects_bad_incoming_side_lengths_and_values(self) -> None:
        ...
        for sides in invalid_sides:
            with self.subTest(sides=sides), self.assertRaisesRegex(
                ValueError, "incoming_gate_sides"
            ):
        ...
        for track_name, track in TRACKS.items():
            ...
            for previous_index in range(len(gates)):
                ...
                with self.subTest(
                    ...
                    self.assertEqual(
                        actual, track.incoming_gate_sides[next_index]
                    )

        self.assertEqual(
            TRACKS["uturn"].incoming_gate_sides, (-1, -1, -1, -1)
        )
...
class EstimationTests(unittest.TestCase):
    def test_ekf_handles_yaw_wrap_and_keeps_covariance_psd(self) -> None:
        ...
        np.testing.assert_allclose(
            ekf.covariance, ekf.covariance.T, atol=1e-12
        )
        ...
        for track in TRACKS:
            for seed in range(5):
                with self.subTest(track=track, seed=seed):
                    ...
                    self.assertTrue(result.evaluation.completed)
                    self.assertLess(
                        result.evaluation.mean_gate_error_m or np.inf, 0.08
                    )
        ...
        with TemporaryDirectory() as temporary:
            ...
            metadata = ReproDatasetGenerator(
                ...
                intrinsics=CameraIntrinsics(
                    width=8, height=6, fx=5, fy=5, cx=4, cy=3
                ),
                seed=7,
            ...
            self.assertEqual([record["sample_id"] for record in records], list(range(5)))
            self.assertEqual(
                [record["pose"] for record in records], metadata["poses"]
            )
