...
from quadpilot.datasets.gate_sampling import project_world_point_to_image
...
from quadpilot.perception.npe import PoseNormalizer, decode_predictions
...
from quadpilot.simulation.visual_loop import TorchNPEPredictor, run_visual_closed_loop
...
class RendererTransformContractTests(unittest.TestCase):
    ...
    def test_world_point_projection_agrees_before_and_after_dataparser(self) -> None:
        ...
        expected_camera = np.array([-0.2, 0.1, -1.2]) * self.transform.scale
        for pose in circle_boundary_poses():
            with self.subTest(pose=pose.tolist()):
                body_c2w = pose_to_camera_matrix(pose)
                point_world = body_c2w[:3, :3] @ np.array([1.2, 0.2, 0.1]) + pose[:3]
                direct = project_world_point_to_image(pose, point_world, intrinsics)
                ...
                u_px = (
                    intrinsics.fx * camera_coordinates[0] / -camera_coordinates[2]
                    + intrinsics.cx
                )
                v_px = (
                    intrinsics.fy * -camera_coordinates[1] / -camera_coordinates[2]
                    + intrinsics.cy
                )
...
class NPELabelContractTests(unittest.TestCase):
    def test_normalizer_decodes_original_world_meters_and_wrapped_yaw(self) -> None:
        ...
        normalizer = PoseNormalizer(
            mean=tuple(
                (low + high) / 2.0 for low, high in (bounds.x, bounds.y, bounds.z)
            ),
            std=(1.3, 1.7, 0.4),
        )
        for pose in circle_boundary_poses():
    ...
    def test_render_label_decode_controller_chain_stays_in_original_world(self) -> None:
        ...
        class EncodedPoseModel(torch.nn.Module):
            ...
            def forward(self, images: torch.Tensor) -> torch.Tensor:
                return (
                    self.output.to(images.device).unsqueeze(0).expand(len(images), -1)
                )
        ...
        class LabelOracleRenderer:
            ...
            def render_rgb(self, camera_to_world: np.ndarray) -> np.ndarray:
                ...
                yaw = float(np.arctan2(camera_to_world[1, 0], camera_to_world[0, 0]))
        ...
        yaw_error = np.arctan2(
            np.sin(
                actual.observations[:, 3] - actual.states[: len(actual.observations), 6]
            ),
            np.cos(
                actual.observations[:, 3] - actual.states[: len(actual.observations), 6]
            ),
        ...
        np.testing.assert_allclose(
            actual.controls, expected.controls, rtol=0.0, atol=1e-5
        )
        ...
        with TemporaryDirectory() as temporary:
            ...
            path.write_text(
                json.dumps(
                    {
                        "transform": np.c_[
                            np.diag([2.0, 1.0, 1.0]), np.zeros(3)
                        ].tolist(),
                        "scale": 1.0,
                    }
                ),
