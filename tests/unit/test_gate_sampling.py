...
from quadpilot.datasets.generation import BASE_DATASET_BOUNDS, CameraIntrinsics
...
class GateFocusedSamplingTests(unittest.TestCase):
    def test_samples_stay_in_bounds_and_on_incoming_side(self) -> None:
        ...
        for _ in range(1000):
            ...
            yaw_error = float((pose[5] - expected_yaw + np.pi) % (2 * np.pi) - np.pi)
    ...
    def test_default_sampler_is_margin_visible_and_gate_balanced_on_all_tracks(
        self,
    ) -> None:
            ...
            with self.subTest(track=track_name):
                ...
                for _ in range(4000):
                    ...
                    projection = project_world_point_to_image(
                        sample.pose,
                        np.asarray(track.gates[gate_name].center),
                        intrinsics,
                    )
                    self.assertTrue(projection.is_visible(intrinsics, margin_px=32.0))
