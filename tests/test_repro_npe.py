...
from scripts.train_repro_npe import _maybe_save_best_checkpoint
from PIL import Image
from torch.utils.data import Dataset

from quadpilot_repro.npe import (
    DatasetRecord,
    MetricAccumulator,
    NPEModel,
    PoseLossConfig,
...
def _write_dataset(root: Path, count: int, *, legacy: bool = False) -> None:
    ...
    for index in range(count):
        pose = [float(index), -float(index) / 2.0, 0.1 * index, 0.0, 0.0, -math.pi + index * 0.2]
        poses.append(pose)
        ...
        rows.append({"sample_id": index, "image": relative, "pose": pose, "attempt": index + 1})
    metadata = {
        "schema_version": 2,
        "n_frames": count,
        "track": root.name,
...
class DatasetIndexTests(unittest.TestCase):
    def test_label_safe_manifest_is_indexed_and_content_hashed(self) -> None:
        with TemporaryDirectory() as temporary:
            ...
            pixels = np.asarray(Image.open(first.records[0].image_path).convert("RGB")).copy()
            pixels[0, 0, 0] = 99
...
class SplitManifestTests(unittest.TestCase):
    def test_split_is_deterministic_disjoint_complete_and_source_balanced(self) -> None:
        with TemporaryDirectory() as temporary:
            ...
            self.assertEqual(set.union(*sets), {record.key for record in dataset.records})
            ...
            base_records = filter_records_by_source_ids(test_records, dataset, [base_source])
            gate_records = filter_records_by_source_ids(test_records, dataset, [gate_source])
            self.assertEqual(len(base_records), 2)
            self.assertEqual(len(gate_records), 1)
            self.assertTrue(all(record.source_id == base_source for record in base_records))
            self.assertTrue(all(record.source_id == gate_source for record in gate_records))
            ...
            with self.assertRaisesRegex(ValueError, "duplicate source_id"):
                filter_records_by_source_ids(test_records, dataset, [base_source, base_source])
    ...
    def test_best_checkpoint_retains_initialization_until_strict_improvement(self) -> None:
        with TemporaryDirectory() as temporary:
    ...
    def test_existing_split_rejects_modified_dataset(self) -> None:
        with TemporaryDirectory() as temporary:
            ...
            pixels = np.asarray(Image.open(root / "images" / "frame_00000.png").convert("RGB")).copy()
            pixels[0, 0, :] = 123
...
class PoseSemanticsTests(unittest.TestCase):
    ...
    def test_legacy_output_layer_affine_conversion_preserves_physical_xyz(self) -> None:
        ...
        normalized_output = converted["regressor.7.weight"] @ features + converted["regressor.7.bias"]
        decoded_position = normalizer.decode_positions(normalized_output[:3].unsqueeze(0))[0]
...
class ArchitectureAndTrainingTests(unittest.TestCase):
    def test_rng_restore_normalizes_map_location_tensors_to_cpu(self) -> None:
        ...
        cpu_serialized.detach.return_value.cpu.return_value = torch.arange(16, dtype=torch.uint8)
        cuda_serialized = mock.MagicMock(spec=torch.Tensor)
        cuda_serialized.detach.return_value.cpu.return_value = torch.arange(24, dtype=torch.uint8)
    ...
    def test_none_weights_never_invokes_download_and_output_shape_is_legacy_five(self) -> None:
        with mock.patch("torch.hub.download_url_to_file", side_effect=AssertionError("network download")):
