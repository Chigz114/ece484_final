...
from quadpilot_repro.npe import build_dataset_index
from scripts import generate_repro_gate_dataset, generate_repro_npe_dataset
...
class ReproDatasetResumeTests(unittest.TestCase):
    ...
    def test_interrupted_resume_is_byte_exact_and_npe_index_equivalent(self) -> None:
        with TemporaryDirectory() as temporary:
            ...
            first_index = build_dataset_index(
                [uninterrupted], fingerprint_mode="full"
            )
    ...
    def test_first_resume_invocation_and_fresh_nonresume_both_work(self) -> None:
        with TemporaryDirectory() as temporary:
            ...
            self.assertEqual(directory_bytes(resume_first), directory_bytes(ordinary_first))
    ...
    def test_pose_annotation_attempt_and_progress_tampering_fail_closed(self) -> None:
        with TemporaryDirectory() as temporary:
            ...
            self.assert_resume_rejected(
                progress_count_tamper, "successes do not match"
            )
...
class ResumeCliTests(unittest.TestCase):
    def test_uniform_and_gate_clis_accept_explicit_resume(self) -> None:
        ...
        for module in (generate_repro_npe_dataset, generate_repro_gate_dataset):
            with self.subTest(module=module.__name__), patch.object(sys, "argv", required):
