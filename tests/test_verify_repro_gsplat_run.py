...
from scripts import verify_repro_gsplat_run as verifier
...
class SyntheticRecoveredRun:
    def __init__(self, root: Path) -> None:
        ...
        image_bytes = sum(receipt_files[name]["size_bytes"] for name in ("images/a.png", "images/b.png"))
        ...
        self.train_run = self.training_output / "lemniscate" / "splatfacto" / self.run_id
...
""",
        ...
        (self.run / "method-plugin-audit.json").write_text(audit_json + "\n", encoding="utf-8")
...
class RecoveredGsplatRunTests(unittest.TestCase):
    ...
    def test_valid_artifacts_preserve_failed_wrapper_as_separate_dimension(self) -> None:
        before = self.fixture.status.read_bytes()
        report = self.fixture.verify()
        self.assertEqual(report["classification"], "WRAPPER_FAILED_TRAINING_ARTIFACTS_VERIFIED")
    ...
    def test_status_env_is_authoritative_when_provenance_has_no_terminal_fields(self) -> None:
    ...
    def test_requires_exactly_one_expected_checkpoint(self) -> None:
        ...
        with self.assertRaisesRegex(verifier.VerificationError, "exactly step-000029999"):
            self.fixture.verify()
    ...
    def test_fails_closed_on_training_exception_even_with_finished_marker(self) -> None:
        ...
        docker_log.write_text(
            docker_log.read_text(encoding="utf-8") + "Traceback (most recent call last):\nRuntimeError: boom\n",
            encoding="utf-8",
        )
        with self.assertRaisesRegex(verifier.VerificationError, "fatal training marker"):
            self.fixture.verify()
    ...
    def test_fails_closed_when_cpu_and_training_plugin_audits_differ(self) -> None:
        ...
        with self.assertRaisesRegex(verifier.VerificationError, "plugin policy changed"):
            self.fixture.verify()

    def test_fails_closed_on_source_or_lpips_digest_change(self) -> None:
        (self.fixture.data / "images" / "a.png").write_bytes(b"tampered")
        with self.assertRaisesRegex(verifier.VerificationError, "source size differs|source SHA-256"):
            self.fixture.verify()
    ...
    def test_fails_closed_on_config_semantic_change(self) -> None:
        ...
        self.fixture.config.write_text(text.replace("num_downscales: 2", "num_downscales: 0"), encoding="utf-8")
    ...
    def test_cli_failure_does_not_materialize_a_report(self) -> None:
        ...
        with (
            ...
            result = verifier.main(
                [str(self.fixture.run), "--track", "lemniscate", "--write-recovered-postflight"]
            )
...
def _fake_checkpoint(nonfinite: str | None = None, bad_shape: str | None = None) -> dict[str, object]:
...
class TorchCheckpointInspectorTests(unittest.TestCase):
    ...
    def test_validates_step_shape_count_cpu_and_finite(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with mock.patch.dict("sys.modules", {"torch": self._fake_torch(_fake_checkpoint())}):
        ...
        self.assertTrue(all(item["finite"] for item in report["gaussian_tensors"].values()))
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            for payload, message in (
                ...
                with self.subTest(message=message):
                    with mock.patch.dict("sys.modules", {"torch": self._fake_torch(payload)}):
