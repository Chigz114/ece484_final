...
SCRIPT = REPO_ROOT / "scripts" / "reproduce" / "gsplat_docker.sh"
IMAGE_REF = (
...
class StaticDockerWrapperContractTests(unittest.TestCase):
    ...
    def test_container_has_no_implicit_pull_or_network_and_data_is_read_only(
        self,
    ) -> None:
    ...
    def test_source_profile_is_explicit_and_both_profiles_are_frozen(self) -> None:
        ...
        self.assertIn(
            "--track is required; choose exactly lemniscate or uturn", self.source
        )
        self.assertIn("data directory basename", self.source)
        for name, value in (
            (
                "LEMNISCATE_RECEIPT_SHA256",
    ...
    def test_cpu_preflight_pins_versions_and_only_two_known_pip_deviations(
        self,
    ) -> None:
    ...
    def test_embedded_preflight_python_compiles_and_receipt_hashes_every_file(
        self,
    ) -> None:
        guard = embedded_heredoc(self.source, "METHOD_PLUGIN_GUARD_PY")
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(
                summary["verified_bytes"], arguments["expected_total_bytes"]
            )
    ...
    def test_cache_home_preserves_the_image_user_site_and_executable_path(self) -> None:
        self.assertIn(
            "readonly IMAGE_EDITABLE_SOURCE='/home/user/nerfstudio'", self.source
        )
        self.assertIn(
...
@unittest.skipIf(
    os.name == "nt" or shutil.which("bash") is None, "requires a POSIX bash"
)
class MockedDockerWrapperBehaviorTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        for data_dir, track in (
            ...
            (data_dir / ".quadpilot_source_receipt.json").write_text(
                json.dumps({"schema_version": 1, "track": track, "files": {}}) + "\n",
                encoding="utf-8",
            )
        self.lpips_checkpoint = (
            self.cache / "nerfstudio-ff0107a7db96" / LPIPS_ALEXNET_RELATIVE_PATH
        )
    ...
    def test_all_training_modes_build_only_the_expected_dry_run(self) -> None:
        ...
        for index, (mode, iterations) in enumerate(cases):
            with self.subTest(mode=mode):
                ...
                self.assertIn(
                    "--pipeline.datamanager.camera-res-scale-factor 0.5", command
                )
                ...
                self.assertIn(
                    "from nerfstudio.scripts.train import entrypoint", command
                )
        ...
        calls = self.docker_log.read_text(encoding="utf-8").splitlines()
        self.assertFalse(
            any(line == "run" or line.startswith("run ") for line in calls)
        )
    ...
    def test_cpu_preflight_failure_blocks_gpu_checks_and_training(self) -> None:
        ...
        self.assertFalse(
            (
                self._run_dir("smoke-1", "preflight-failed") / "nvidia-smi.before.txt"
            ).exists()
        )
        runs = [
            line
