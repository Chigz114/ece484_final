...
from quadpilot.cli import data_download as downloader
...
class StrictSelectionTests(unittest.TestCase):
    ...
    def test_real_run_verifies_roots_before_listing_and_records_hashes(self) -> None:
        with TemporaryDirectory() as temporary:
            ...
            self.assertEqual(
                (track_dir / "transforms.json").read_bytes(), self.transforms
            )
            ...
            first_list = next(
                i for i, event in enumerate(client.events) if event[0] == "list"
            )
            root_opens = [
                i
