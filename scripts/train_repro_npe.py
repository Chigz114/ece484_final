...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--backbone", choices=("resnet18", "resnet34", "resnet50"), default="resnet50")
    parser.add_argument(
        "--weights",
    ...
    parser.add_argument("--epochs", type=int, default=100, help="Total epochs, including resumed epochs")
    parser.add_argument("--batch-size", type=int, default=8, help="Physical device batch size")
...
def repository_provenance() -> dict[str, Any]:
    ...
    return {
        ...
        "tracked_code_sha256": {
            "quadpilot_repro/npe.py": sha256_file(REPOSITORY_ROOT / "quadpilot_repro" / "npe.py"),
            "scripts/train_repro_npe.py": sha256_file(Path(__file__).resolve()),
        },
    }
...
def main() -> int:
    ...
    if not args.resume and any((output_dir / name).exists() for name in ("last_npe.pth", "best_npe.pth")):
    ...
    print(f"Indexing {len(args.data_dir)} dataset source(s) with {args.fingerprint_mode!r} fingerprints...")
    ...
    if args.resume:
        ...
        if resume_checkpoint["split_manifest"].get("manifest_sha256") != split_manifest.get("manifest_sha256"):
        ...
        if stored_config != run_config:
            differing = sorted(key for key in set(stored_config) | set(run_config) if stored_config.get(key) != run_config.get(key))
        ...
        best_val_position_cm = float(resume_checkpoint.get("best_val_position_cm", math.inf))
        ...
        weight_initialization = copy.deepcopy(
            resume_checkpoint.get("provenance", {}).get(
                ...
                {"identifier": resume_checkpoint["model_config"].get("initial_weights", "unknown")},
            )
        )
        checkpoint_lineage = list(
            resume_checkpoint.get("provenance", {}).get("checkpoint_lineage", [])
        ) + [{"operation": "resume", "path": str(resume_path), "sha256": initial_checkpoint_hash}]
        ...
        model, normalizer, stored_preprocess, stored_loss = model_from_checkpoint(init_checkpoint, device=device)
        ...
        if stored_preprocess != preprocess or stored_loss != loss_config:
            raise ValueError("fine-tune preprocessing/loss must match the initialization checkpoint")
        ...
        if not isinstance(legacy_state, dict):
            raise ValueError("legacy checkpoint does not contain a model state dictionary")
        if args.weights != "none":
            raise ValueError("--weights must be none when --init-legacy-checkpoint is used")
        ...
        model_config = copy.deepcopy(model.config)
        weight_initialization = torchvision_weight_provenance(args.backbone, args.weights)
        normalizer_provenance = {
            "strategy": "fit_frozen_training_split",
            "dataset_fingerprint": dataset.fingerprint,
            "split_manifest_sha256": split_manifest["manifest_sha256"],
    ...
    if args.resume:
        ...
        if strategy == "fit_frozen_training_split":
            ...
            if not _same_normalizer(normalizer, fitted):
                raise ValueError("resume training-position normalizer differs despite matching dataset")
        elif strategy != "inherited_initial_checkpoint":
            raise ValueError(f"unsupported resume normalizer provenance strategy: {strategy!r}")
        ...
        "initial_checkpoint": (
            {
                "path": str(
                    Path(
                        args.resume or args.init_checkpoint or args.init_legacy_checkpoint
                    ).expanduser().resolve()
                ),
                "sha256": initial_checkpoint_hash,
    ...
    if args.init_checkpoint or args.init_legacy_checkpoint:
        ...
        if not saved:
            raise AssertionError("initialization baseline was not retained as the first candidate")
        print(
    ...
    if start_epoch >= args.epochs:
        raise ValueError(f"checkpoint already completed {start_epoch} epochs (target={args.epochs})")
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if best_val_position_cm != next_best_val_position_cm:
            raise AssertionError("best-checkpoint decision disagrees with checkpoint payload")
        if args.save_every and (epoch + 1) % args.save_every == 0:
            atomic_torch_save(output_dir / f"checkpoint_epoch_{epoch + 1:04d}.pth", last_payload)
        atomic_json_save(
    ...
    print(f"Training complete. Best validation position error: {best_val_position_cm:.3f} cm")
