...
DEFAULT_MANIFEST = (
    Path(__file__).resolve().parents[1] / "repro_assets" / "manifest.json"
)
    ...
    *,
    ...
    with client.open_content(remote.file_id, start=offset) as response:
        ...
        if append:
            ...
            raise AssetDownloadError(
                f"Drive returned HTTP {status} for {remote.path}"
            )
...
def referenced_basenames(transforms_data: bytes) -> list[str]:
    ...
    for index, frame in enumerate(frames):
        if not isinstance(frame, Mapping) or not isinstance(frame.get("file_path"), str):
