...
from quadpilot_repro.tracks import TRACKS  # noqa: E402
from quadpilot_repro.visual_loop import (  # noqa: E402
    TorchNPEPredictor,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--estimator", choices=("raw", "ekf", "both"), default="both"
    )
...
def _manifest_run(track: str) -> str:
    manifest_path = REPOSITORY_ROOT / "repro_assets" / "manifest.json"
    manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    return str(manifest["tracks"][track]["run"])


def _resolve_assets(args: argparse.Namespace) -> tuple[Path, Path, Path]:
    run = _manifest_run(args.track)
    run_dir = (
        REPOSITORY_ROOT / "outputs" / args.track / "splatfacto" / run
    )
    renderer_checkpoint = (
    ...
    npe_checkpoint = (
        args.npe_checkpoint
        or REPOSITORY_ROOT / TRACKS[args.track].model_path
    )
    resolved = tuple(
...
def main() -> int:
    ...
    renderer_checkpoint, dataparser_transform, npe_checkpoint = _resolve_assets(
        args
    )
