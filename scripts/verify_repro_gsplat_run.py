...
def _parse_shell_env(path: Path) -> dict[str, str]:
    ...
    for line_number, raw in enumerate(path.read_text(encoding="utf-8").splitlines(), 1):
        ...
        _require(match is not None, f"invalid shell record line {line_number} in {path.name}")
        key, encoded = match.groups()
        _require(key not in result, f"duplicate shell record key {key!r} in {path.name}")
        try:
...
def _same_path(actual: Path, expected: Path, name: str) -> None:
    ...
    _require(actual_resolved == expected_resolved, f"{name} does not resolve to {expected}")
...
def _require_no_symlink_components(path: Path, stop: Path | None = None) -> None:
    ...
    for component in reversed(components):
        _require(not component.is_symlink(), f"symlink is forbidden in audited path: {component}")
...
def _safe_source_member(root: Path, relative: str) -> Path:
    ...
    _require(not pure.is_absolute(), f"absolute receipt path is forbidden: {relative!r}")
    _require(
    ...
    try:
        ...
        raise VerificationError(f"receipt path escapes source root: {relative}") from exc
    return path
...
def _extract_prefixed_json(text: str, prefix: str, source: str) -> dict[str, Any]:
    ...
    _require(len(matches) == 1, f"{source} must contain exactly one {prefix.strip()} record")
    try:
        payload = json.loads(
            matches[0],
            parse_constant=_reject_constant,
        ...
        raise VerificationError(f"cannot parse {prefix.strip()} in {source}: {exc}") from exc
    _require(isinstance(payload, dict), f"{prefix.strip()} in {source} must be an object")
    return payload


def _validate_method_audit(payload: Mapping[str, Any], source: str) -> None:
    _require(payload.get("policy") == "built-in-only", f"{source} plugin policy changed")
    _require(
    ...
    for index, row in enumerate(rows):
        _require(isinstance(row, dict), f"{source} entry point {index} must be an object")
        _require(
...
def _verify_source(
    ...
    *,
    ...
    _require(data_root.name == profile.name, "source data directory basename/profile mismatch")
    ...
    _require(receipt.get("schema_version") == 1, "source receipt schema_version must be 1")
    _require(receipt.get("track") == profile.name, "source receipt track/profile mismatch")
    files = receipt.get("files")
    _require(isinstance(files, dict), "source receipt files must be an object")
    _require(len(files) == profile.files, "source receipt file count differs from frozen profile")
    ...
    for index, (relative, metadata) in enumerate(files.items(), 1):
        ...
        _require(isinstance(metadata, dict), f"receipt metadata must be an object: {relative}")
        size = _integer(metadata.get("size_bytes"), f"receipt size_bytes for {relative}")
        ...
        _require(path.stat().st_size == size, f"source size differs from receipt: {relative}")
        _require(_sha256(path) == expected_hash, f"source SHA-256 differs from receipt: {relative}")
        total_bytes += size
        pure = PurePosixPath(relative)
        if len(pure.parts) == 2 and pure.parts[0] == "images" and pure.suffix.lower() == ".png":
            image_paths.add(relative)
            image_bytes += size
        if progress and (index % 100 == 0 or index == len(files)):
            print(f"SOURCE_HASH_PROGRESS {index}/{len(files)}", file=sys.stderr, flush=True)

    _require(len(image_paths) == profile.images, "source image count differs from frozen profile")
    _require(image_bytes == profile.image_bytes, "source image bytes differ from frozen profile")
    _require(total_bytes == profile.total_bytes, "source total bytes differ from frozen profile")
    _require(
    ...
    for index, frame in enumerate(frames):
        ...
        _require(isinstance(value, str), f"transforms frame {index} file_path is invalid")
        frame_paths.append(PurePosixPath(value).as_posix())
    _require(len(frame_paths) == profile.images, "transforms camera count differs from profile")
    _require(len(set(frame_paths)) == len(frame_paths), "transforms contains duplicate image paths")
    _require(set(frame_paths) == image_paths, "transforms images differ from the receipt image set")
    ...
    _require(len(vertices) == 1, "sparse_pc.ply must declare exactly one vertex element")
    try:
        sparse_points = int(vertices[0].split()[2])
    except (IndexError, ValueError) as exc:
        raise VerificationError("sparse_pc.ply vertex count is invalid") from exc
    _require(sparse_points == profile.sparse_points, "sparse point count differs from profile")
    ...
    _require(summary == expected_summary, "saved receipt-verification.json disagrees with source")
    return {
        "status": "PASS",
...
def _verify_preflight_and_plugins(run_root: Path, profile: SourceProfile) -> dict[str, Any]:
    ...
    _require(preflight_text.count("PREFLIGHT_OK") == 1, "preflight did not finish exactly once")
    ...
    pip_check = _extract_prefixed_json(preflight_text, "PIP_CHECK ", preflight_path.name)
    _require(
    ...
    _require(saved_builtins == builtins, "saved built-in method audit differs from preflight")
    ...
    _require(saved_cpu_audit == cpu_audit, "saved CPU plugin audit differs from preflight")
...
def _yaml_get(payload: Mapping[str, Any], *keys: str) -> Any:
    current: Any = payload
    for key in keys:
        _require(isinstance(current, dict) and key in current, f"config.yml is missing {'.'.join(keys)}")
        current = current[key]
    return current
...
def _verify_config(config_path: Path, track: str, run_id: str) -> dict[str, Any]:
    try:
        ...
        raise VerificationError("PyYAML is required to inspect config.yml on CPU") from exc
    try:
        config = yaml.load(config_path.read_text(encoding="utf-8"), Loader=yaml.BaseLoader)
    except Exception as exc:
        raise VerificationError(f"cannot parse config.yml with non-instantiating BaseLoader: {exc}") from exc
    ...
    for keys, expected in expected_scalars.items():
        ...
        _require(actual == expected, f"config.yml {'.'.join(keys)}={actual!r}, expected {expected!r}")
    _require(_yaml_get(config, "data") == ["/", "data"], "config.yml data is not /data")
    _require(_yaml_get(config, "output_dir") == ["/", "outputs"], "config.yml output_dir is not /outputs")
    _require(
...
def _verify_command(
    ...
    *,
    ...
    for fragment in required:
        _require(fragment in normalized, f"command.sh is missing frozen fragment: {fragment}")
    return {"status": "PASS", **_file_record(command_path, relative_to=command_path.parent)}


def _verify_image_inspect(path: Path) -> dict[str, Any]:
    payload = _load_json_value(path)
    _require(isinstance(payload, list) and len(payload) == 1, "image inspect must contain one image")
    image = payload[0]
...
def inspect_torch_checkpoint(path: Path, expected_step: int) -> dict[str, Any]:
    ...
    _require(isinstance(step, int) and not isinstance(step, bool), "checkpoint step is not an integer")
    ...
    for name, tail_shape in GAUSSIAN_TAIL_SHAPES.items():
        ...
        _require(bool(getattr(tensor, "is_floating_point")()), f"{key} is not floating point")
        device = str(getattr(tensor, "device", ""))
        _require(device == "cpu", f"{key} was not loaded on CPU")
        try:
            finite = bool(torch.isfinite(tensor).all().item())
        except Exception as exc:
            raise VerificationError(f"cannot check finiteness for {key}: {exc}") from exc
...
def _verify_existing_artifact_manifest(
    ...
    for line in path.read_text(encoding="utf-8").splitlines():
        ...
        actual[name] = digest
    _require(actual == expected, "training-artifacts.sha256 disagrees with actual artifacts")
...
def verify_run(
    ...
    *,
    ...
    _require(track in SOURCE_PROFILES or profile is not None, f"unsupported track profile: {track}")
    ...
    _require(not recovery_path.exists(), "recovered-postflight.json already exists; refusing overwrite")
    ...
    _require(status.get("result") == "failed", "recovery verifier requires original wrapper result=failed")
    try:
        wrapper_exit_code = int(status.get("exit_code", ""))
    except ValueError as exc:
        raise VerificationError("status.env exit_code is not an integer") from exc
    _require(wrapper_exit_code > 0, "recovery verifier requires a nonzero wrapper exit code")
    ...
    for key, expected in required_provenance.items():
        _require(provenance.get(key) == expected, f"provenance {key!r} differs from {expected!r}")
    if "source_profile" in provenance:
        _require(provenance["source_profile"] == track, "provenance source_profile/track mismatch")
    ...
    for key, expected in optional_profile_counts.items():
        if key in provenance:
            _require(provenance[key] == str(expected), f"provenance {key} differs from profile")
    ...
    if "exit_code" in provenance:
        _require(provenance["exit_code"] == str(wrapper_exit_code), "status/provenance exit code mismatch")
    ...
    _require(lpips_record["size_bytes"] == LPIPS_SIZE_BYTES, "LPIPS AlexNet size changed")
    ...
    _require(train_run_dir.is_dir(), f"missing Nerfstudio run directory: {train_run_dir}")
    ...
    checkpoint_files = sorted(checkpoint_dir.glob("step-*.ckpt")) if checkpoint_dir.is_dir() else []
    _require(
    ...
    _require(
        isinstance(checkpoint_report, dict) and checkpoint_report.get("status") == "PASS",
    ...
    _require(status_bytes_after == status_bytes_before, "status.env changed during verification")
    ...
    artifact_records = {
        ...
        "preflight_log": _file_record(run_root / "preflight-container.log", relative_to=run_root),
...
def write_recovery_report(run_root: Path, report: Mapping[str, Any]) -> Path:
    ...
    _require(report.get("overall_success") is False, "recovery report must not claim overall success")
    training = report.get("training_artifacts")
    wrapper = report.get("wrapper_status")
    _require(isinstance(training, dict) and training.get("status") == "PASS", "artifacts are not PASS")
    _require(isinstance(wrapper, dict) and wrapper.get("status") == "failed", "wrapper is not failed")
    target = run_root / "recovered-postflight.json"
    _require(not target.exists(), "recovered-postflight.json already exists; refusing overwrite")
    serialized = (json.dumps(report, indent=2, sort_keys=True, allow_nan=False) + "\n").encode("utf-8")
    fd, temporary_name = tempfile.mkstemp(prefix=".recovered-postflight.", suffix=".tmp", dir=run_root)
    ...
    try:
        ...
        try:
            ...
            raise VerificationError("recovered-postflight.json appeared concurrently; refusing overwrite") from exc
        temporary.unlink()
        try:
...
def _parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("run_root", type=Path, help="existing failed-wrapper train-30k run root")
    parser.add_argument(
        "--track",
        required=True,
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        return 2
    except Exception as exc:  # fail closed without an unaudited traceback in normal CLI use
        print(f"VERIFY_REPRO_GSPLAT_RUN_FAILED: unexpected {type(exc).__name__}: {exc}", file=sys.stderr)
        return 2
