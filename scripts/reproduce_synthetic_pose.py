...
from quadpilot_repro.simulation import SimulationResult, run_pose_simulation
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--yaw-noise-std-deg", type=float, default=1.0
    )
    parser.add_argument(
        "--crossing-hysteresis", type=float, default=0.05
    )
    parser.add_argument(
        "--output",
        type=Path,
...
def run_metrics(result: SimulationResult) -> dict[str, float | bool | int | None]:
    ...
    residual_steps = np.linalg.norm(
        np.diff(estimate - truth, axis=0), axis=1
    )
    control_changes = np.linalg.norm(
        np.diff(result.controls, axis=0), axis=1
    )
    return {
...
def aggregate(runs: list[dict[str, object]]) -> dict[str, object]:
    ...
    summary: dict[str, object] = {
        ...
        "total_runs": len(runs),
        "completion_rate": sum(bool(run["completed"]) for run in runs)
        / len(runs),
    }
